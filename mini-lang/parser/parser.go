...
func (p *Parser) parseCallExpression(left ast.Expression) ast.Expression {
	ce := &ast.CallExpression{Token: p.curToken, Function: left}
	ce.Arguments = p.parseExpressionList(token.RPAREN)
	return ce
}

func (p *Parser) parseIndexExpression(left ast.Expression) ast.Expression {
	ci := &ast.IndexExpression{Token: p.curToken, Left: left}

	p.nextToken()
	ci.Index = p.parseExpression(LOWEST)
...
// Check if peek is of a TokenType p.
// If so, advance to next token.
// If no, append peek error.
